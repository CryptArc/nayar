  ...
  */
    ...
    try{
      handler.init(function(err, results){
        ...
        handler.get({
          ...
          if(err) return cb(err);
          layer = layer[0];
          if(!layer) return cb(getErrorResponse(new Error("invalid request"),query));
          handler.get({
            table : "Poi",
            ...
            getPOIsData(pois, query, function(err, filledPois){
              if(err) return cb(err);
              if(handler !== null){
                handler.disconnect();
              }
  ...
  function getPOIsData(pois, query, cb){
    ...
    pois.forEach(function (poi, index){
      ...
      (function (poi) {
        getPOIData(poi, function (err, results){
          if(err) return cb(err);
