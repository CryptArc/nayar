...
var nayar = function(){
  ...
  function getRawPOIs(conn, params, layer, cb){
    ...
    if(poiType === "geo"){
      ...
      qstr += 'FROM Poi WHERE poiType = "geo" AND layerID = '+layerID+' LIMIT 0, 50;';
    } else if (poiType === "vision"){
      qstr += 'referenceImage, anchorID, objectID, transformID, animationID ';
      qstr += 'FROM Poi WHERE poiType = "vision" AND layerID = '+layerID+' LIMIT 0, 50;';
    } else {
      return cb(new Error("invalid poiType in database: only geo or vision are allowed!"), null, null);
    }
    // make query
    console.log(qstr);
  ...
  function makeQueries(pois, params){
    ...
    pois.forEach(function(poi){
      ...
      q += "interpolationParam, persist, 'repeat', 'from', 'to', axis_x, axis_y, ";
      q += 'axis_z FROM Animation WHERE poiID = "'+poi.id+'";';
  ...
  function formatHotspots(rawPois){
    ...
    rawPois.forEach(function(rawpoi, index, array){
      ...
      if(rawpoi.showSmallBiw !== null) poi.showSmallBiw = rawpoi.showSmallBiw;
      if(rawpoi.showBiwOnClick !== null) poi.showBiwOnClick = rawpoi.showBiwOnClick;
      ...
      if(rawpoi.inFocus !== null) poi.inFocus = rawpoi.inFocus;
    ...
    function filterPOI(poi, pois){
        if(poi.anchor.referenceImage !== null){
          ...
          if(params.recognizedReferenceImage){
            return poi.anchor.referenceImage === params.recognizedReferenceImage ? true : false;
          } else {
            return false;
          }
          ...
          var poi = _.find(pois, function(anchorpoi){return anchorpoi.id === poi.anchor.poi});
